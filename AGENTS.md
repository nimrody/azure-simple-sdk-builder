...
- `sdk/src/main/java/com/azure/simpleSDK/network/models/` - Contains auto-generated Network API model classes
- `sdk/src/main/java/com/azure/simpleSDK/network/client/` - Contains auto-generated Network SDK client classes
- `sdk/src/main/java/com/azure/simpleSDK/compute/models/` - Contains auto-generated Compute API model classes
- `sdk/src/main/java/com/azure/simpleSDK/compute/client/` - Contains auto-generated Compute SDK client classes
...
The code generation system has been refactored to support multiple Azure services with **separate SDK packages**:

- **`SpecLoader.generateSeparateSDKs()`** - Generates separate SDKs for each Azure service with distinct packages
- **Modular Architecture** - Each Azure service gets its own package namespace and client class
- **Service Isolation** - Complete separation between Network, Compute, and future services

### Separate SDK Structure

The system now generates separate SDKs for each Azure service:

**Network SDK:**
- Package: `com.azure.simpleSDK.network.*` 
- Client: `AzureNetworkClient`
- Models: `com.azure.simpleSDK.network.models.*`

**Compute SDK:**
- Package: `com.azure.simpleSDK.compute.*`
- Client: `AzureComputeClient` 
- Models: `com.azure.simpleSDK.compute.models.*`

### Adding New Azure Services

To add support for additional Azure services:

1. Add a new `ServiceSpec` entry to the `SERVICES` list in `SpecLoader.java`
2. Specify the service display name, specs path, and package structure
3. The system will automatically generate separate packages and client classes

**Current Status:**
- ✅ **Network APIs (2024-07-01)** - 726 operations, 1,110 definitions in `com.azure.simpleSDK.network.*`
- ✅ **Compute APIs (2024-11-01)** - 184 operations, 431 definitions in `com.azure.simpleSDK.compute.*`  
- ✅ **Separate Clients** - `AzureNetworkClient` and `AzureComputeClient` with full type safety
- ✅ **Demo Integration** - Working demonstration using both separate clients

**Architecture Benefits:**
1. **Service Isolation** - No conflicts between different Azure service types
2. **Modular Usage** - Applications can depend only on needed Azure services
3. **Clean Namespaces** - Each service has its own package hierarchy
4. **Independent Evolution** - Services can be updated independently

Example service configuration:
```java
private static final List<ServiceSpec> SERVICES = Arrays.asList(
    new ServiceSpec("Network", 
...
7. **Output**: Generated code is organized into service-specific packages:
   - Network Models: `sdk/src/main/java/com/azure/simpleSDK/network/models/`
   - Network Client: `sdk/src/main/java/com/azure/simpleSDK/network/client/`
   - Compute Models: `sdk/src/main/java/com/azure/simpleSDK/compute/models/`
   - Compute Client: `sdk/src/main/java/com/azure/simpleSDK/compute/client/`
   - HTTP Layer: `sdk/src/main/java/com/azure/simpleSDK/http/`
...
- **Service-Specific Packages**: Separate packages for each Azure service (network, compute, etc.) with isolated namespaces
...
```java
// Network Models (for complex types)
package com.azure.simpleSDK.network.models;

import com.fasterxml.jackson.annotation.JsonProperty;
import java.util.*;

// Generated from filename.json:123
public record NetworkModelName(
    String property1,
...
// Compute Models (for complex types)
package com.azure.simpleSDK.compute.models;

import com.fasterxml.jackson.annotation.JsonProperty;
import java.util.*;

// Generated from ComputeRP.json:456
public record ComputeModelName(
    String property1,
    List<String> zones
) {
}
...
// Generated from filename.json:789
public enum NetworkEnumName {
    VALUE1("Value1"),
    VALUE2("Value2");

    private final String value;

    NetworkEnumName(String value) {
        this.value = value;
    }
...
- **Network SDK**: 726 operations, 1,110 definitions in `com.azure.simpleSDK.network.*`
- **Compute SDK**: 184 operations, 431 definitions in `com.azure.simpleSDK.compute.*`
- **Total**: 910 GET operation methods across both service clients
- **Separate Clients**: `AzureNetworkClient` and `AzureComputeClient` with full type safety
...
```java
// Enable strict mode for separate clients
AzureNetworkClient networkClient = new AzureNetworkClient(credentials, true);
AzureComputeClient computeClient = new AzureComputeClient(credentials, true);
...
```java
// Create credentials and separate service clients
AzureCredentials credentials = new ServicePrincipalCredentials(clientId, clientSecret, tenantId);
AzureNetworkClient networkClient = new AzureNetworkClient(credentials);
AzureComputeClient computeClient = new AzureComputeClient(credentials);

// Call Network service methods with full type safety and documentation
// Pagination is handled automatically for list operations
AzureResponse<VirtualNetworkGatewayListConnectionsResult> networkResponse = networkClient
    .listConnectionsVirtualNetworkGateways(subscriptionId, resourceGroupName, gatewayName);

// Call Compute service methods with separate client
AzureResponse<VirtualMachineListResult> computeResponse = computeClient
    .listAllVirtualMachines(subscriptionId, statusOnly, filter, expand);

VirtualNetworkGatewayListConnectionsResult connections = networkResponse.getBody();
VirtualMachineListResult virtualMachines = computeResponse.getBody();
```
...
- **Separate Client Usage**: Demonstrates both `AzureNetworkClient` and `AzureComputeClient`
- **Multiple Resource Types**: Tests Azure Firewalls, Virtual Networks, Network Interfaces, and Virtual Machines
- **Cross-Service Testing**: Shows how to use different Azure services in the same application
- **Pagination Testing**: Exercises automatic pagination across different list operations for both services
...
Using separate Network and Compute SDK clients
==========================================
...
VM Response Status: 200
Number of virtual machines found: 5
...
=========================
VM Properties: Available
Provisioning State: SUCCEEDED
VM Size: Standard_D2s_v3
Computer Name: vm-web-01
Zones: 1
[... additional VMs ...]
```
...
- **Separate Client Usage**: Creating both `AzureNetworkClient` and `AzureComputeClient` with proper credentials and strict mode options
- **Cross-Service API Calls**: Using Network methods like `listAllAzureFirewalls(subscriptionId)` and Compute methods like `listAllVirtualMachines(subscriptionId, statusOnly, filter, expand)`
- **Service Isolation**: Demonstrates how different Azure services work independently with their own clients
- **Pagination Testing**: Shows automatic pagination handling across both Network and Compute resource types
- **Response Processing**: Accessing typed response data from both service types
- **Resource Parsing**: Extracting resource group names from Azure resource IDs across services
- **Strict Mode**: Optional strict mode for unknown property detection and validation across both clients
- **Error Handling**: Graceful handling of authentication and network errors for multiple services
...
✅ **Separate Service SDKs**: Complete separation of Network and Compute APIs with distinct packages  
✅ **Network SDK**: 726 operations, 1,110 definitions in `com.azure.simpleSDK.network.*`  
✅ **Compute SDK**: 184 operations, 431 definitions in `com.azure.simpleSDK.compute.*`  
✅ **External Reference Resolution**: Cross-file and cross-directory references across services  
✅ **AllOf Inheritance**: Full OpenAPI schema composition support  
✅ **Type Safety**: Separate `AzureNetworkClient` and `AzureComputeClient` with full type safety  
...
✅ **Cross-Service Demo**: Working demonstration with both Network and Compute resources  
✅ **Comprehensive Testing**: Unit test suite with 100% pass rate covering pagination, single-value requests, and error scenarios  
✅ **Production Ready**: Zero compilation errors, tested with live Azure APIs across multiple services  

### Current Limitations

⏳ **Operation Support**: Only GET operations currently implemented  
⏳ **API Coverage**: Limited to Azure Network (2024-07-01) and Compute (2024-11-01) APIs  
